#!/usr/bin/env python3
"""
Calculate Abregelung.Z column for all 366 rows in WSData.

Formula from Excel: =WENN(O157/I157<=Abregelung;0;O157-P157/EtaStromGas)
Where:
- O = ueberschuss_strom
...
IF(ueberschuss_strom / stromverbr_raumwaerm_korr <= 1.0):
    abregelung = 0
ELSE:
    abregelung = ueberschuss_strom - (einspeich / 0.65)
"""
...
ABREGELUNG_THRESHOLD = 1.0  # n2: Base Rate (100%)

def calculate_abregelung():
    """Calculate Abregelung for all 366 rows"""
    
    print("=" * 80)
    print("CALCULATING ABREGELUNG.Z (Column R) FOR ALL 366 ROWS")
    print("=" * 80)
    print(f"\nConstants from WS1 Annual Electricity Diagram:")
    print(f"  n1 (EtaStromGas): {ETA_STROM_GAS * 100}% = {ETA_STROM_GAS}")
    print(f"  n2 (Abregelung threshold): {ABREGELUNG_THRESHOLD * 100}% = {ABREGELUNG_THRESHOLD}")
    print()
    ...
    for row in daily_rows:
        ...
        # Calculate abregelung
        if stromverbr == 0:
            # Avoid division by zero
            abregelung_value = 0
        else:
            ratio = ueberschuss / stromverbr
            if ratio <= ABREGELUNG_THRESHOLD:
                # Ratio is within limit, no curtailment
                abregelung_value = 0
            else:
                # Ratio exceeds limit, calculate curtailment
                abregelung_value = ueberschuss - (einspeich / ETA_STROM_GAS)
        
        # Update the row
        row.abregelung_z = round(abregelung_value, 2)
        row.save()
        
        updated_count += 1
        daily_sum += abregelung_value
        
        # Show sample calculations for first 3 days
        if row.tag_im_jahr <= 3:
            ratio_str = f"{ratio:.4f}" if stromverbr != 0 else "N/A"
            condition = "ratio <= 1.0" if stromverbr != 0 and ratio <= ABREGELUNG_THRESHOLD else "ratio > 1.0"
            print(f"Row {row.tag_im_jahr:3d}: Überschuss={ueberschuss:12,.2f}, Stromverbr={stromverbr:12,.2f}")
            print(f"          Einspeich={einspeich:12,.2f}, Ratio={ratio_str:>10s}")
            print(f"          {condition} → Abregelung.Z = {abregelung_value:,.2f}")
            print()
    
    # Row 366 = sum of rows 1-365
    try:
        row_366 = all_rows.get(tag_im_jahr=366)
        row_366.abregelung_z = round(daily_sum, 2)
        row_366.save()
        updated_count += 1
        
        print(f"Row 366: Sum of days 1-365")
        print(f"          Abregelung.Z = {daily_sum:,.2f}")
        print()
    except WSData.DoesNotExist:
        pass
    
    print("=" * 80)
    print(f"✓ Successfully calculated Abregelung.Z for {updated_count} rows")
    print(f"✓ Sum of Abregelung.Z (rows 1-365): {daily_sum:,.2f}")
    print(f"✓ Row 366 Abregelung.Z (sum): {round(daily_sum, 2):,.2f}")
    print("=" * 80)

if __name__ == "__main__":
    calculate_abregelung()
